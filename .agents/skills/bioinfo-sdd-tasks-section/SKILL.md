...
2. Convert the approved section spec and plan into ordered implementation and
   validation tasks.
3. Keep the list dependency-ordered: section contract checks first, then
   installed-ref and `bioinfo_tool` context checks, then wrapper/adapter work,
   then evidence/report tasks and gate updates.
4. Include pack resolution, section validation, wrapper checks, adapter checks,
   dependency-aware execution tests, evidence recording, and gate updates.
5. Keep tasks bounded to the declared section. Do not add upstream preprocessing
   or downstream interpretation tasks unless the section contract is revised.
6. Run `PYTHONPATH=src python -m bioinfo_sdd validate-section <section_id>`.
7. Stop at `task_review` unless it is already approved.
