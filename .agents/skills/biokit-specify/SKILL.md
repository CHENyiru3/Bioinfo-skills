...
The text the user typed after `/biokit-specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
3. **Create the spec feature directory**:
   ...
   **Create the directory and spec file**:
   ...
   - Persist the resolved path to `.specify/feature.json`:
     ...
     This allows downstream commands (`/biokit-plan`, `/biokit-tasks`, etc.) to locate the feature directory without relying on git branch name conventions.

   **IMPORTANT**:
   - You must only create one feature per `/biokit-specify` invocation
...
7. **Specification Quality Validation**: After writing the initial spec, validate it against quality criteria:
   ...
   a. **Create Spec Quality Checklist**: Generate a checklist file at `SPECIFY_FEATURE_DIRECTORY/checklists/requirements.md` using the checklist template structure with these validation items:
      ...
      **Feature**: [Link to spec.md]
      ...
      - Items marked incomplete require spec updates before `/biokit-clarify` or `/biokit-plan`
      ```
...
8. **Report completion** to the user with:
   ...
   - Readiness for the next phase (`/biokit-clarify` or `/biokit-plan`)
